...
/*(function (sockjs) {
 ...
 sockjs.sendtoserv = function (command, data) {
 var cmd = JSON.stringify({
 command: command,
 data: data
 });
 console.log('send cmd:',cmd);
 window.sockjsem = sockjs;
 sockjs.send(cmd);
 }
 })(sockjs);*/
...
/*
 ...
 sockjs.sendtoserv('setSessionId', Math.round(Math.random() * 10000));
 });
 sockjs.on('message', function(data) {
 console.log('i get message ', data);
 print('[.] message', data);
 });
 sockjs.on('disconnect', function()  {print('[*] close');});
 */
